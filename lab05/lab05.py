...
class LinkedList:
    ...
    def cursor_insert(self, value):
        ...
        ### BEGIN SOLUTION
        new = self.cursor
        n = LinkedList.Node(value, prior=new, next=new.next)
        new.next = new.next.prior= self.cursor = n
        self.length += 1
        ### END SOLUTION
    ...
    def __eq__(self, other):
        ...
        ### BEGIN SOLUTION
        if type(other) is not LinkedList or other.length != self.length:
            return False
        selfnode = self.head.next
